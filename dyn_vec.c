...
{
	dyn_vec_t * vec = (dyn_vec_t *)malloc(sizeof(dyn_vec_item));
	assert(vec);
...
{
	dyn_vec_t* tmp = vec;
	if(! (tmp->arr = realloc(tmp->arr, num_elements * tmp->elem_size)) )
	{
		fprintf(stderr, "realloc failure FILE:%s	func:%s LINE:%d\n",__FILE__, __func__, __LINE__);
		return(-1);
	}	
	else
	{
...
void DynVecPrint(dyn_vec_t* vec)
{
	...
	for( ; i < size_tmp; ++i)
	{
		...
		{
		case 4:
			printf("%d ", ((int *)vec->arr)[e_size * i]);
			break;
		case 2:
			printf("%d ", ((short *)vec->arr)[e_size * i]);
			break;
		case 1:
			printf("%c ", ((char *)vec->arr)[e_size * i]);
			break;
		default:
